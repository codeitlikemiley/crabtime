    ...
    var body: some View {
        Button(action: store.showWorkspacePalette) {
            HStack(spacing: 8) {
                ...
                Text(currentRecord?.displayTitle ?? "RustGoblin")
                    .lineLimit(1)
        ...
        Button {
            ...
            HStack(spacing: 12) {
                Image(systemName: sourceKindSymbol(record.sourceKind))
                    .foregroundStyle(isSelected(record) ? RustGoblinTheme.Palette.panelTint : RustGoblinTheme.Palette.textMuted)
                    .frame(width: 16)
            ...
            .background(
                RoundedRectangle(cornerRadius: 14, style: .continuous)
                    .fill(isSelected(record) ? RustGoblinTheme.Palette.selectionFill : RustGoblinTheme.Palette.buttonFill)
            )
            .overlay {
                RoundedRectangle(cornerRadius: 14, style: .continuous)
                    .stroke(isSelected(record) ? RustGoblinTheme.Palette.strongDivider : RustGoblinTheme.Palette.divider, lineWidth: 1)
            }
        }
        .buttonStyle(.plain)
        .interactivePointer()
...
private struct DifficultyFilterChip: View {
    ...
    var body: some View {
        Button(action: action) {
            Text(title)
                .font(.caption.weight(.semibold))
                .foregroundStyle(isSelected ? RustGoblinTheme.Palette.ink : tint.opacity(0.85))
                .padding(.horizontal, 10)
                .padding(.vertical, 6)
                .background(
                    Capsule()
                        .fill(isSelected ? tint.opacity(0.18) : RustGoblinTheme.Palette.buttonFill)
                )
                .overlay {
                    Capsule()
                        .stroke(isSelected ? tint.opacity(0.7) : RustGoblinTheme.Palette.divider, lineWidth: 1)
                }
        }
        .buttonStyle(.plain)
        .interactivePointer()
