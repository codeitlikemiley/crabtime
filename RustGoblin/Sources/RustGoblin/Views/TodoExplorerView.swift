            ...
            if items.isEmpty {
                VStack(spacing: 8) {
                    Image(systemName: "checkmark.circle")
                        ...
                        .foregroundStyle(RustGoblinTheme.Palette.moss.opacity(0.6))
                    ...
                    .font(.caption)
                    .foregroundStyle(RustGoblinTheme.Palette.textMuted.opacity(0.7))
                }
    ...
    private func scopeButton(title: String, isActive: Bool, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            Text(title)
                ...
                .foregroundStyle(isActive ? RustGoblinTheme.Palette.ink : RustGoblinTheme.Palette.textMuted)
                .padding(.horizontal, 10)
                .padding(.vertical, 4)
                .background(
                    Capsule()
                        .fill(isActive ? RustGoblinTheme.Palette.ember.opacity(0.25) : Color.white.opacity(0.06))
                )
                .overlay(
                    Capsule()
                        .stroke(isActive ? RustGoblinTheme.Palette.ember.opacity(0.4) : Color.clear, lineWidth: 1)
                )
        }
        .buttonStyle(.plain)
        .interactivePointer()
...
private struct TodoItemRow: View {
    ...
    var body: some View {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                HStack(spacing: 6) {
                    Text(item.fileName)
                        ...
                        .foregroundStyle(RustGoblinTheme.Palette.cyan.opacity(0.8))
        ...
        .background(
            RoundedRectangle(cornerRadius: 8, style: .continuous)
                .fill(isSelected ? RustGoblinTheme.Palette.ember.opacity(0.15) : Color.clear)
        )
        .overlay(
            RoundedRectangle(cornerRadius: 8, style: .continuous)
                .stroke(isSelected ? RustGoblinTheme.Palette.ember.opacity(0.3) : Color.clear, lineWidth: 1)
        )
        .interactivePointer()
    }
    ...
    private var iconColor: Color {
        switch item.kind {
        case .todoMacro: return RustGoblinTheme.Palette.ember
        case .todoComment, .todoDocComment: return RustGoblinTheme.Palette.cyan
