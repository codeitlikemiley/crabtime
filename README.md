...
RustGoblin provides a focused context-rich environment that strips away the complexity of traditional IDEs. It replaces generic AI web chat with deeply integrated coding features designed *specifically* for Rust students: 
...
![RustGoblin](.github/preview.png)

## Installation

RustGoblin requires **macOS 14.0+** and relies on system Rust dependencies. 

1. Download the latest `.dmg` release from the [Releases](https://github.com/goldcoders/rustgoblin/releases) page.
2. Drag `RustGoblin.app` to your Applications folder.
3. On first launch, if you lack `rustc`, `cargo`, or the `exercism` CLI, RustGoblin's Setup Wizard will automatically download and install these dependencies locally.

### Manual Building

If you wish to build RustGoblin from source:
...
The resulting `RustGoblin.app` bundle will exist in `.build/release/`.
...
Currently, RustGoblin is focused primarily on the macOS experience given its AppKit foundations. To learn more about its internals and how we manage processes, state, and terminal emulation, read the architecture documentation.
