...
struct RustlingsWorkspaceScaffolder {
    ...
    func createChallengeStub(
        ...
        try appendCargoBinEntriesIfNeeded(slug: normalizedSlug, to: workspaceRootURL.appendingPathComponent("Cargo.toml", isDirectory: false))
    ...
    func enrichChallengeInBackground(
        ...
        Task { @MainActor in
            ...
            let (exerciseContent, exerciseGenerated) = await generateExerciseContent(
                title: result.title,
                workspaceRootPath: result.workspaceRootURL.path,
            ...
            try? updateInfoEntryTestFlagIfNeeded(
                slug: result.slug,
                hasTests: Self.containsRustTests(exerciseContent),
                infoTomlURL: result.workspaceRootURL.appendingPathComponent("info.toml", isDirectory: false)
            )
    ...
    private func appendInfoEntryIfNeeded(slug: String, title: String, to infoTomlURL: URL) throws {
        let existing = (try? String(contentsOf: infoTomlURL, encoding: .utf8)) ?? canonicalInfoTomlHeader
        ...
        let normalizedExisting = Self.removingEmptyExercisesPlaceholder(from: existing)
        let separator = normalizedExisting.hasSuffix("\n") || normalizedExisting.isEmpty ? "" : "\n"
        try (normalizedExisting + separator + entry).write(to: infoTomlURL, atomically: true, encoding: .utf8)
    }

    private func appendCargoBinEntriesIfNeeded(slug: String, to cargoTomlURL: URL) throws {
        let existing = (try? String(contentsOf: cargoTomlURL, encoding: .utf8)) ?? canonicalCargoToml
        let exerciseEntry = "  { name = \"\(slug)\", path = \"exercises/\(slug).rs\" },"
        let solutionEntry = "  { name = \"\(slug)_sol\", path = \"solutions/\(slug).rs\" },"
        guard !existing.contains(exerciseEntry), !existing.contains(solutionEntry) else { return }

        let lines = existing.components(separatedBy: "\n")
        let updatedLines: [String]

        if let binStartIndex = lines.firstIndex(where: { $0 == "bin = [" }),
           let binEndIndex = lines[(binStartIndex + 1)...].firstIndex(where: { $0 == "]" }) {
            updatedLines = Array(lines[..<binEndIndex]) + [exerciseEntry, solutionEntry] + Array(lines[binEndIndex...])
        } else {
            let canonicalLines = canonicalCargoToml.components(separatedBy: "\n")
            guard let binStartIndex = canonicalLines.firstIndex(where: { $0 == "bin = [" }),
                  let binEndIndex = canonicalLines[(binStartIndex + 1)...].firstIndex(where: { $0 == "]" }) else {
                throw NSError(
                    domain: "RustlingsWorkspaceScaffolder",
                    code: 1,
                    userInfo: [NSLocalizedDescriptionKey: "The canonical Cargo.toml template is missing the bin list."]
                )
            }
            updatedLines = Array(canonicalLines[..<binEndIndex]) + [exerciseEntry, solutionEntry] + Array(canonicalLines[binEndIndex...])
        }

        try updatedLines.joined(separator: "\n").write(to: cargoTomlURL, atomically: true, encoding: .utf8)
    }

    private func updateInfoEntryTestFlagIfNeeded(slug: String, hasTests: Bool, infoTomlURL: URL) throws {
        let existing = try String(contentsOf: infoTomlURL, encoding: .utf8)
        let lines = existing.components(separatedBy: "\n")
        var updatedLines: [String] = []
        var insideTargetEntry = false
        var didWriteFlag = false
        ...
        for line in lines {
            ...
            if trimmed == "name = \"\(slug)\"" {
                insideTargetEntry = true
                updatedLines.append(line)
                continue
            }

            if insideTargetEntry && trimmed.hasPrefix("test = ") {
                if !didWriteFlag {
                    updatedLines.append("test = \(hasTests ? "true" : "false")")
                    didWriteFlag = true
                }
                continue
            }

            if insideTargetEntry && !didWriteFlag && trimmed.hasPrefix("hint = ") {
                updatedLines.append("test = \(hasTests ? "true" : "false")")
                didWriteFlag = true
            }
        ...
        try updatedLines.joined(separator: "\n").write(to: infoTomlURL, atomically: true, encoding: .utf8)
    }
    ...
    private func shouldRepairInfoToml(_ existing: String) -> Bool {
        existing.contains("# Crab Time challenge workspace")
            || !existing.contains("format_version =")
            || (!existing.contains("exercises = []") && !existing.contains("[[exercises]]"))
    }
    ...
    static func containsRustTests(_ text: String) -> Bool {
        text.contains("#[cfg(test)]") || text.contains("#[test]")
    }

    static func removingEmptyExercisesPlaceholder(from text: String) -> String {
        text
            .components(separatedBy: "\n")
            .filter { $0.trimmingCharacters(in: .whitespaces) != "exercises = []" }
            .joined(separator: "\n")
            .trimmingCharacters(in: .whitespacesAndNewlines)
    }
    ...
    private func readme(title: String) -> String {
        ...
        A \(AppBrand.longName) workspace for authoring custom Rustlings-style exercises.
        """
    }
